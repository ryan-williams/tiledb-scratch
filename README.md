# tiledb{,soma} import order segfault (macOS)

On macOS, `tiledbsoma.Experiment.open` segfaults when `tiledb` is imported before `tiledbsoma`:
```bash
IMPORT_TILEDB=  python segfault-repro.py  # ✅ OK
IMPORT_TILEDB=1 python segfault-repro.py  # ❌ segfault when `tiledb` is imported before `tiledbsoma`
```

More stack trace:
```bash
IMPORT_TILEDB=1 python -X faulthandler segfault-repro.py
...
#   File "$PWD/segfault-repro.py", line 10 in <module>
# 
